...
import { ScatterChartCategories } from '@/components/client/_types/scatter-categories';
...
export default function ScatterChart({
  ...
  const categories: ScatterChartCategories = {
    Gesellschaft: {
      color: categoriesInfo.find(cat => cat.categoryName === "Gesellschaft")?.categoryColor || "black",
      borderColor: categoriesInfo.find(cat => cat.categoryName === "Gesellschaft")?.categoryBorderColor || "black",
      x: [], y: [], text: [], textPosition: [], hoverText: [],
    },
    Wirtschaft: {
      color: categoriesInfo.find(cat => cat.categoryName === "Wirtschaft")?.categoryColor || "black",
      borderColor: categoriesInfo.find(cat => cat.categoryName === "Wirtschaft")?.categoryBorderColor || "black",
      x: [], y: [], text: [], textPosition: [], hoverText: [],
    },
    Umwelt: {
      color: categoriesInfo.find(cat => cat.categoryName === "Umwelt")?.categoryColor || "black",
      borderColor: categoriesInfo.find(cat => cat.categoryName === "Umwelt")?.categoryBorderColor || "black",
      x: [], y: [], text: [], textPosition: [], hoverText: [],
    },
  };
  ...
  const createPlotData = (): Partial<PlotData>[] => {
    xCoordinates.forEach((x, index) => {
      ...
      if (duplicateIndices.length > 1) {
        ...
        // Position logic similar to original
        switch (offsetIndex) {
          case 0: textPosition = "top center"; break;
          case 1: textPosition = "bottom center"; yOffset = -0.05; break;
          case 2: textPosition = "left center"; xOffset = -0.05; break;
          case 3: textPosition = "right center"; xOffset = 0.05; break;
    ...
    return Object.keys(categories).map((key) => ({
      ...
      type: "scatter" as any,
      name: key,
      textposition: categories[key as CategoryKey].textPosition as any,
      textfont: {
